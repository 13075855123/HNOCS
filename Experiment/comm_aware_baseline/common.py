"""Shared helpers for the CommAware-Heuristic baseline runner.

The baseline extraction, mappable conversion, cost weights, and grouped metrics
mirror the B-2 runner's evaluation schema.  This module is intentionally local
to the CommAware baseline so later baselines can evolve independently.
"""
...
class OriginalReference:
    """Original static mapping evaluation and B-2-normalized reference."""
...
def extract_original_assignment(graph: TaskGraph) -> dict[int, int]:
    """Extract Original static mapping using the same rule as B-2."""
    return {
...
def make_original_static_tasks_mappable(graph: TaskGraph) -> None:
    """Turn original non-GB static tasks into B-2-style mappable tasks."""
...
def validate_assignment(
    ...
    """Validate a complete task-to-PE assignment for this baseline."""
    mappable = set(graph.mappable_task_ids)
    keys = set(assignment)
    missing = sorted(mappable - keys)
    extra = sorted(keys - mappable)
    if missing or extra:
        raise ValueError(
    ...
    for tid, pe in assignment.items():
        if graph.tasks[tid].is_gb_task:
            raise ValueError(f"GB task {tid} must not be assigned")
        if not isinstance(pe, int) or pe < 0 or pe >= num_pes:
            raise ValueError(f"task {tid} has invalid PE {pe}; valid range is 0..{num_pes - 1}")
...
def config_payload(
    ...
    """Return serializable common configuration metadata."""
    ...
    if cost_reference is not None:
        payload["cost_reference"] = asdict(cost_reference)
    return payload
...
def write_run_summaries(output_dir: Path, records: list[dict[str, object]]) -> None:
    """Write compact cross-workload summaries."""
    output_dir.mkdir(parents=True, exist_ok=True)
    write_json(output_dir / "aggregate_summary.json", records)
    if not records:
...
def metric_value(metrics: dict[str, object], section: str, key: str) -> float:
    value = metrics.get(section, {})
    if isinstance(value, dict):
        leaf = value.get(key, 0.0)
