"""Shared helpers for the ThermalGreedy baseline runner.

The helpers in this module keep the runner aligned with the B-2 evaluation
schema while leaving the ThermalGreedy search objective independent from
OMNeT++ final metrics.
"""
...
class BaselineReference:
    """Original mapping evaluation and normalized cost reference."""
...
def validate_assignment(
    ...
    if missing:
        raise ValueError(f"missing assignments for mappable tasks: {missing}")
    if extra:
...
def pe_to_coord(pe: int, cols: int) -> tuple[int, int]:
...
def manhattan_hops(pe_a: int, pe_b: int, cols: int) -> int:
    if pe_a == pe_b:
        return 0
    r1, c1 = pe_to_coord(pe_a, cols)
    r2, c2 = pe_to_coord(pe_b, cols)
...
def baseline_temperature_factors(scalars: OmnetScalars) -> dict[int, float]:
    """Build PE temperature factors from one Original static observation."""
...
def guard_output_path(
    ...
    if any(part.startswith("B-2") for part in rel_parts):
        raise RuntimeError("refusing to write ThermalGreedy output into any out/B-2* path")
    ...
    for benchmark in benchmarks:
        for method in ("original", "thermal_greedy"):
            for name in common_files + metric_files:
                candidate = output_dir / benchmark / method / name
                if candidate.exists():
                    collisions.append(candidate)
        proxy_path = output_dir / benchmark / "thermal_greedy" / "proxy.json"
        if proxy_path.exists():
            collisions.append(proxy_path)
    ...
    if collisions:
        ...
        raise RuntimeError(
            "refusing to overwrite existing ThermalGreedy outputs. "
