"""CLI runner for ThermalRC-LS / Thermal-Resistance-Aware Local Search."""

from __future__ import annotations

import argparse
import json
import sys
import time
from dataclasses import asdict
from pathlib import Path
from typing import Any

_HERE = Path(__file__).resolve().parent
_EXP = _HERE.parent
_PROJ = _EXP.parent

for _d in (_HERE, _EXP, _PROJ):
    if str(_d) not in sys.path:
        sys.path.insert(0, str(_d))

from mapping.csv_writer import write_static_csv
from mapping.omnet_cost_model import SimParams
from mapping.task_graph import TaskGraph

from common import (
    BENCHMARKS,
    CostWeights,
    OmnetRunConfig,
    baseline_temperature_factors,
    build_cost_model,
    build_omnet_evaluator,
    cost_from_metrics,
    dataclass_dict,
    evaluate_original_reference,
    extract_original_assignment,
    grouped_metrics,
    guard_output_path,
    make_original_static_tasks_mappable,
    require_valid_scalars,
    validate_assignment,
    write_csv_rows,
    write_json,
)
from thermal_rc_ls_mapper import ThermalRCLSMapper, ThermalRCLSSearchConfig
from thermal_rc_proxy import (
    RCObjectiveWeights,
    RCProxyConfig,
    calibrate_or_synthetic_R,
    power_vector_rows,
    proxy_score,
    resistance_matrix_rows,
    task_power_proxy,
)


def main(argv: list[str] | None = None) -> int:
    parser = build_parser()
    args = parser.parse_args(argv)
    try:
        benchmarks = selected_benchmarks(args)
        params = SimParams(rows=args.rows, cols=args.cols)
        if params.num_pes != 16:
            raise ValueError("ThermalRC-LS is currently scoped to the 4x4 / 16 PE setup")

        proxy_config = RCProxyConfig(
            rows=args.rows,
            cols=args.cols,
            Tambient=params.Tambient,
            T_hot=args.T_hot if args.T_hot is not None else params.Tthrottle,
            power_idle=args.power_idle,
            power_compute=args.power_compute,
            leakage_base_power=args.leakage_base_power,
            ridge_lambda=args.ridge_lambda,
            synthetic_self_R=args.synthetic_self_R,
            synthetic_decay=args.synthetic_decay,
        )
        objective_weights = build_objective_weights(args)
        search_config = ThermalRCLSSearchConfig(
            seed=args.seed,
            init_mode=args.init,
            max_iter=args.max_iter,
            no_improve_patience=args.patience,
            time_limit_s=args.time_limit,
            hot_pe_count=args.hot_pes,
            cool_pe_count=args.cool_pes,
            candidate_task_limit=args.candidate_task_limit,
            random_swap_rate=default_random_swap_rate(args),
            random_candidate_count=args.random_candidates,
            enable_sa=args.sa,
            sa_initial_temp=args.sa_initial_temp,
            sa_cooling=args.sa_cooling,
        )
        proxy_config.validate()
        objective_weights.validate()
        search_config.validate()

        weights = CostWeights(
            w_T=args.w_T,
            w_sigma=args.w_sigma,
            w_hot=args.w_hot,
            w_makespan=args.w_makespan,
            w_H=args.w_H,
            w_congestion=args.w_congestion,
            w_D=args.w_D,
            w_L=args.w_L,
            w_E=args.w_E,
        )
        omnet_config = OmnetRunConfig(
            omnet_bin=args.omnet_bin,
            omnet_ned_paths=args.omnet_ned_paths,
            omnet_workdir=args.omnet_workdir,
            omnet_ini=args.omnet_ini,
            omnet_base_config=args.omnet_base_config,
            omnetpp_root=args.omnetpp_root,
            omnet_timeout_s=args.omnet_timeout,
            verbose=args.verbose,
        )
        baseline_temp_payload = load_baseline_temp_json(args.baseline_temp_json)
        output_dir = Path(args.out)

        if args.dry_run:
            dry_run(benchmarks, params, proxy_config, objective_weights, search_config, args, omnet_config, output_dir)
            return 0

        guard_output_path(
            _PROJ,
            output_dir,
            benchmarks,
            force=args.force,
            will_write_metrics=not args.proxy_only,
        )

        records: list[dict[str, Any]] = []
        for benchmark in benchmarks:
            csv_path = resolve_csv_path(benchmark)
            baseline_temp = baseline_temp_for_workload(baseline_temp_payload, benchmark)
            if args.proxy_only:
                record = run_proxy_only(
                    benchmark,
                    csv_path,
                    output_dir,
                    params,
                    proxy_config,
                    objective_weights,
                    search_config,
                    baseline_temp,
                    calibration_mode=args.calibration,
                )
            else:
                record = run_full_workload(
                    benchmark,
                    csv_path,
                    output_dir,
                    params,
                    weights,
                    omnet_config,
                    proxy_config,
                    objective_weights,
                    search_config,
                    baseline_temp,
                    calibration_mode=args.calibration,
                )
            records.append(record)

        write_summaries(output_dir, records)
        print(f"\nWrote {method_label(objective_weights)} results to {output_dir.resolve()}")
        return 0
    except Exception as exc:
        parser.exit(1, f"ERROR: {exc}\n")


def run_proxy_only(
    benchmark: str,
    csv_path: Path,
    output_dir: Path,
    params: SimParams,
    proxy_config: RCProxyConfig,
    objective_weights: RCObjectiveWeights,
    search_config: ThermalRCLSSearchConfig,
    baseline_temperature: list[float] | None,
    calibration_mode: str,
) -> dict[str, Any]:
    start = time.perf_counter()
    graph = TaskGraph.from_csv(csv_path)
    original_assignment = extract_original_assignment(graph)
    make_original_static_tasks_mappable(graph)
    validate_assignment(graph, original_assignment, params.num_pes)

    task_power = task_power_proxy(graph, proxy_config)
    baseline_power = proxy_score(
        graph,
        original_assignment,
        task_power,
        [[0.0 for _ in range(params.num_pes)] for _ in range(params.num_pes)],
        proxy_config,
        objective_weights,
    )["power_W"]
    calibration_temperature = None if calibration_mode == "synthetic" else baseline_temperature
    resistance_matrix, calibration = calibrate_or_synthetic_R(
        proxy_config,
        baseline_power,
        calibration_temperature,
    )

    mapper = ThermalRCLSMapper(
        graph,
        original_assignment,
        task_power,
        resistance_matrix,
        proxy_config,
        objective_weights,
        search_config,
    )
    result = mapper.run()
    validate_assignment(graph, result.assignment, params.num_pes)

    workload_dir = output_dir / benchmark
    original_dir = workload_dir / "original"
    method_key = method_dir_name(objective_weights)
    rc_dir = workload_dir / method_key
    original_dir.mkdir(parents=True, exist_ok=True)
    rc_dir.mkdir(parents=True, exist_ok=True)

    write_static_csv(
        graph,
        original_assignment,
        original_dir / "mapping.csv",
        comment="Original static mapping for ThermalRC-LS proxy reference",
    )
    write_static_csv(
        graph,
        result.assignment,
        rc_dir / "mapping.csv",
        comment=f"{method_label(objective_weights)} mapping; proxy-only output",
    )
    write_static_csv(
        graph,
        result.assignment,
        rc_dir / "remapped.csv",
        comment=f"{method_label(objective_weights)} remapped; proxy-only output",
    )
    write_method_artifacts(rc_dir, result, resistance_matrix, calibration)
    (original_dir / "summary.txt").write_text(
        f"[{benchmark}] Original mapping written for ThermalRC-LS proxy reference\n",
        encoding="utf-8",
    )
    summary = proxy_summary_text(benchmark, result.proxy)
    (rc_dir / "summary.txt").write_text(summary + "\n", encoding="utf-8")
    print(summary)

    final_proxy = result.proxy[method_key]
    original_proxy = result.proxy["original"]
    return {
        "benchmark": benchmark,
        "mode": "proxy_only",
        "method": method_key,
        "method_label": method_label(objective_weights),
        "init": search_config.init_mode,
        "calibration_source": calibration.source,
        "original_proxy_score": original_proxy["score"],
        "candidate_proxy_score": final_proxy["score"],
        "original_Tmax_proxy": original_proxy["Tmax_proxy"],
        "candidate_Tmax_proxy": final_proxy["Tmax_proxy"],
        "original_SigmaT_proxy": original_proxy["SigmaT_proxy"],
        "candidate_SigmaT_proxy": final_proxy["SigmaT_proxy"],
        "original_HotCount_proxy": original_proxy["HotCount_proxy"],
        "candidate_HotCount_proxy": final_proxy["HotCount_proxy"],
        "original_CommProxy": original_proxy.get("CommProxy", ""),
        "candidate_CommProxy": final_proxy.get("CommProxy", ""),
        "iterations": result.iterations,
        "converged": result.converged,
        "elapsed_s": time.perf_counter() - start,
    }


def run_full_workload(
    benchmark: str,
    csv_path: Path,
    output_dir: Path,
    params: SimParams,
    weights: CostWeights,
    omnet_config: OmnetRunConfig,
    proxy_config: RCProxyConfig,
    objective_weights: RCObjectiveWeights,
    search_config: ThermalRCLSSearchConfig,
    baseline_temperature: list[float] | None,
    calibration_mode: str,
) -> dict[str, Any]:
    start = time.perf_counter()
    graph = TaskGraph.from_csv(csv_path)
    original_assignment = extract_original_assignment(graph)
    make_original_static_tasks_mappable(graph)
    validate_assignment(graph, original_assignment, params.num_pes)

    evaluator = build_omnet_evaluator(omnet_config)
    cost_model = build_cost_model(graph, params, weights)
    if omnet_config.verbose:
        print(f"\n[{benchmark}] Original OMNeT++ simulation...")
    original_ref = evaluate_original_reference(
        graph,
        original_assignment,
        evaluator,
        cost_model,
        params,
        benchmark,
    )

    task_power = task_power_proxy(graph, proxy_config)
    baseline_power = proxy_score(
        graph,
        original_assignment,
        task_power,
        [[0.0 for _ in range(params.num_pes)] for _ in range(params.num_pes)],
        proxy_config,
        objective_weights,
    )["power_W"]
    if calibration_mode == "synthetic":
        calibration_temperature = None
    else:
        calibration_temperature = baseline_temperature or original_ref.scalars.pe_max_temp_K or original_ref.scalars.pe_temps_final_K
    resistance_matrix, calibration = calibrate_or_synthetic_R(
        proxy_config,
        baseline_power,
        calibration_temperature,
    )

    mapper = ThermalRCLSMapper(
        graph,
        original_assignment,
        task_power,
        resistance_matrix,
        proxy_config,
        objective_weights,
        search_config,
        initial_assignment=thermal_greedy_initial(
            graph,
            original_assignment,
            params,
            original_ref.scalars if search_config.init_mode == "thermal_greedy" else None,
            search_config.init_mode,
        ),
    )
    result = mapper.run()
    validate_assignment(graph, result.assignment, params.num_pes)

    label = method_label(objective_weights)
    if omnet_config.verbose:
        print(f"[{benchmark}] {label} final OMNeT++ simulation...")
    rc_scalars = evaluator.evaluate(graph, result.assignment)
    require_valid_scalars(benchmark, label, rc_scalars)
    rc_metrics = grouped_metrics(
        graph,
        result.assignment,
        rc_scalars,
        cost_model,
        params,
        baseline_makespan_s=original_ref.scalars.makespan_s,
    )

    workload_dir = output_dir / benchmark
    original_dir = workload_dir / "original"
    method_key = method_dir_name(objective_weights)
    rc_dir = workload_dir / method_key
    original_dir.mkdir(parents=True, exist_ok=True)
    rc_dir.mkdir(parents=True, exist_ok=True)

    write_static_csv(
        graph,
        original_assignment,
        original_dir / "mapping.csv",
        comment="Original static mapping for ThermalRC-LS reference",
    )
    write_static_csv(
        graph,
        result.assignment,
        rc_dir / "mapping.csv",
        comment=f"{method_label(objective_weights)} mapping",
    )
    write_static_csv(
        graph,
        result.assignment,
        rc_dir / "remapped.csv",
        comment=f"{method_label(objective_weights)} remapped",
    )
    write_method_artifacts(rc_dir, result, resistance_matrix, calibration)

    write_json(
        original_dir / "metrics.json",
        {
            "name": benchmark,
            "method": "original",
            "metrics": original_ref.metrics,
            "config": {
                "source_csv": str(csv_path),
                "weights": dataclass_dict(weights),
                "cost_reference": asdict(original_ref.cost_reference),
                "baseline_temperature_vector_K": original_ref.scalars.pe_max_temp_K,
            },
        },
    )
    write_json(
        rc_dir / "metrics.json",
        {
            "name": benchmark,
            "method": method_key,
            "method_label": method_label(objective_weights),
            "metrics": rc_metrics,
            "proxy": result.proxy,
            "config": {
                "source_csv": str(csv_path),
                "rows": params.rows,
                "cols": params.cols,
                "num_pes": params.num_pes,
                "final_evaluation_weights": dataclass_dict(weights),
                "proxy_config": asdict(proxy_config),
                "objective_weights": asdict(objective_weights),
                "search_config": asdict(search_config),
                "cost_reference": asdict(original_ref.cost_reference),
                "calibration": calibration.as_dict(),
                "not_exact_reproduction": True,
            },
        },
    )
    original_summary = f"[{benchmark}] Original cost={cost_from_metrics(original_ref.metrics):.4f}"
    rc_summary = full_summary_text(benchmark, original_ref.metrics, rc_metrics, result.proxy, calibration.source)
    (original_dir / "summary.txt").write_text(original_summary + "\n", encoding="utf-8")
    (rc_dir / "summary.txt").write_text(rc_summary + "\n", encoding="utf-8")
    print(rc_summary)

    return {
        "benchmark": benchmark,
        "mode": "full",
        "method": method_key,
        "method_label": method_label(objective_weights),
        "init": search_config.init_mode,
        "calibration_source": calibration.source,
        "original_cost": cost_from_metrics(original_ref.metrics),
        "candidate_cost": cost_from_metrics(rc_metrics),
        "original_proxy_score": result.proxy["original"]["score"],
        "candidate_proxy_score": result.proxy[method_key]["score"],
        "original_Tmax_K": original_ref.metrics["thermal"]["T1_pe_peak_temp_K"],
        "candidate_Tmax_K": rc_metrics["thermal"]["T1_pe_peak_temp_K"],
        "original_SigmaT_K": original_ref.metrics["thermal"]["T3_temp_std_K"],
        "candidate_SigmaT_K": rc_metrics["thermal"]["T3_temp_std_K"],
        "original_HotCount": original_ref.metrics["thermal"]["T5_over_throttle_count"],
        "candidate_HotCount": rc_metrics["thermal"]["T5_over_throttle_count"],
        "original_Comm": original_ref.metrics["communication"]["C1_total_comm_cost"],
        "candidate_Comm": rc_metrics["communication"]["C1_total_comm_cost"],
        "iterations": result.iterations,
        "converged": result.converged,
        "elapsed_s": time.perf_counter() - start,
    }


def thermal_greedy_initial(
    graph: TaskGraph,
    original_assignment: dict[int, int],
    params: SimParams,
    original_scalars: Any,
    init_mode: str,
) -> dict[int, int] | None:
    """Use the existing ThermalGreedy implementation when requested."""
    if init_mode != "thermal_greedy":
        return None
    tg_dir = _EXP / "thermal_greedy_baseline"
    inserted = False
    if str(tg_dir) not in sys.path:
        sys.path.insert(0, str(tg_dir))
        inserted = True
    try:
        from thermal_greedy_mapper import ThermalGreedyMapper
        from thermal_proxy import ThermalProxyConfig

        temp_factors = baseline_temperature_factors(original_scalars) if original_scalars else None
        mapper = ThermalGreedyMapper(
            graph,
            ThermalProxyConfig(rows=params.rows, cols=params.cols),
            original_assignment=original_assignment,
            baseline_temperature_factor=temp_factors,
            local_swap_passes=2,
        )
        return mapper.run().assignment
    except Exception:
        return None
    finally:
        if inserted:
            try:
                sys.path.remove(str(tg_dir))
            except ValueError:
                pass


def write_method_artifacts(
    rc_dir: Path,
    result: Any,
    resistance_matrix: list[list[float]],
    calibration: Any,
) -> None:
    result.proxy["calibration"] = calibration.as_dict()
    write_json(rc_dir / "proxy.json", result.proxy)
    write_json(rc_dir / "history.json", result.history)
    write_csv_rows(rc_dir / "rc_matrix.csv", resistance_matrix_rows(resistance_matrix))
    final = result.proxy[result.proxy["method"]]
    write_csv_rows(rc_dir / "power_vector.csv", power_vector_rows(final["power_W"], final["temperatures_K"]))


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run ThermalRC-LS with a lightweight thermal resistance proxy"
    )
    parser.add_argument("--workload", choices=sorted(BENCHMARKS), help="Run one benchmark")
    parser.add_argument("--benchmarks", default="gemm,mpeg4,vopd,hnn")
    parser.add_argument("--csv", help="Path to one static task CSV")
    parser.add_argument("--out", default="out/thermal-rc-ls-v1")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--objective", choices=["rc", "thermal_only"], default="rc")
    parser.add_argument("--init", choices=["original", "thermal_greedy", "random_spread"], default="original")
    parser.add_argument("--proxy-only", action="store_true")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--force", action="store_true")
    parser.add_argument("--verbose", "-v", action="store_true")

    parser.add_argument("--rows", type=int, default=4)
    parser.add_argument("--cols", type=int, default=4)
    parser.add_argument("--calibration", choices=["auto", "synthetic"], default="auto")
    parser.add_argument("--baseline-temp-json", help="Optional JSON list or workload-to-list baseline PE temperature vector")
    parser.add_argument("--T-hot", type=float, default=None)
    parser.add_argument("--power-idle", type=float, default=0.3)
    parser.add_argument("--power-compute", type=float, default=2.5)
    parser.add_argument("--leakage-base-power", type=float, default=0.3)
    parser.add_argument("--ridge-lambda", type=float, default=1e-3)
    parser.add_argument("--synthetic-self-R", type=float, default=2.5)
    parser.add_argument("--synthetic-decay", type=float, default=0.55)

    parser.add_argument("--rc-w-tmax", type=float, default=0.55)
    parser.add_argument("--rc-w-sigma", type=float, default=0.30)
    parser.add_argument("--rc-w-hot", type=float, default=0.10)
    parser.add_argument("--rc-w-comm", type=float, default=0.05)

    parser.add_argument("--max-iter", type=int, default=300)
    parser.add_argument("--patience", type=int, default=50)
    parser.add_argument("--time-limit", type=float, default=0.0)
    parser.add_argument("--hot-pes", type=int, default=4)
    parser.add_argument("--cool-pes", type=int, default=4)
    parser.add_argument("--candidate-task-limit", type=int, default=4)
    parser.add_argument("--random-swap-rate", type=float, default=None)
    parser.add_argument("--random-candidates", type=int, default=8)
    parser.add_argument("--sa", action="store_true")
    parser.add_argument("--sa-initial-temp", type=float, default=0.05)
    parser.add_argument("--sa-cooling", type=float, default=0.98)

    parser.add_argument("--w-T", type=float, default=1.0)
    parser.add_argument("--w-sigma", type=float, default=1.0)
    parser.add_argument("--w-hot", type=float, default=0.6)
    parser.add_argument("--w-makespan", type=float, default=1.2)
    parser.add_argument("--w-H", type=float, default=0.4)
    parser.add_argument("--w-congestion", type=float, default=0.7)
    parser.add_argument("--w-D", type=float, default=0.4)
    parser.add_argument("--w-L", type=float, default=0.2)
    parser.add_argument("--w-E", type=float, default=0.5)

    parser.add_argument("--omnet-bin", default="D:/HNOCS/libhnocs.exe")
    parser.add_argument("--omnet-ned-paths", default="D:/HNOCS/src;D:/HNOCS/examples/task_driven")
    parser.add_argument("--omnet-workdir", default="D:/HNOCS/examples/task_driven")
    parser.add_argument("--omnet-ini", default="D:/HNOCS/examples/task_driven/omnetpp.ini")
    parser.add_argument("--omnet-base-config", default="ONoCGeneral")
    parser.add_argument("--omnetpp-root", default="D:/omnetpp/omnetpp-6.3.0")
    parser.add_argument("--omnet-timeout", type=float, default=60.0)
    return parser


def selected_benchmarks(args: argparse.Namespace) -> list[str]:
    if args.csv:
        path = Path(args.csv)
        name = path.stem.replace("tasks_", "").replace("_static", "").lower()
        BENCHMARKS[name] = str(path)
        return [name]
    if args.workload:
        return [args.workload.lower()]
    names = [part.strip().lower() for part in args.benchmarks.split(",") if part.strip()]
    if not names:
        raise ValueError("no benchmarks selected")
    unknown = [name for name in names if name not in BENCHMARKS]
    if unknown:
        raise ValueError(f"unknown benchmarks: {unknown}")
    return names


def resolve_csv_path(benchmark: str) -> Path:
    path = Path(BENCHMARKS[benchmark])
    return path if path.is_absolute() else _PROJ / path


def load_baseline_temp_json(path_text: str | None) -> Any:
    if not path_text:
        return None
    path = Path(path_text)
    data = json.loads(path.read_text(encoding="utf-8"))
    return data


def baseline_temp_for_workload(payload: Any, benchmark: str) -> list[float] | None:
    if payload is None:
        return None
    if isinstance(payload, list):
        return [float(value) for value in payload]
    if isinstance(payload, dict):
        value = payload.get(benchmark) or payload.get(benchmark.upper())
        if isinstance(value, list):
            return [float(item) for item in value]
    return None


def dry_run(
    benchmarks: list[str],
    params: SimParams,
    proxy_config: RCProxyConfig,
    objective_weights: RCObjectiveWeights,
    search_config: ThermalRCLSSearchConfig,
    args: argparse.Namespace,
    omnet_config: OmnetRunConfig,
    output_dir: Path,
) -> None:
    print("Planned ThermalRC-LS runs:")
    print(f"  output={output_dir.resolve()}")
    print(f"  mode={'proxy_only' if args.proxy_only else 'full'}")
    print(f"  calibration={args.calibration}")
    print(f"  rows={params.rows} cols={params.cols} num_pes={params.num_pes}")
    print(f"  proxy_config={json.dumps(asdict(proxy_config), sort_keys=True)}")
    print(f"  objective_weights={json.dumps(asdict(objective_weights), sort_keys=True)}")
    print(f"  search_config={json.dumps(asdict(search_config), sort_keys=True)}")
    print(f"  OMNeT++ runs={0 if args.proxy_only else len(benchmarks) * 2}")
    if not args.proxy_only:
        print("  path checks:")
        for label, path in [
            ("omnet_bin", omnet_config.omnet_bin),
            ("omnet_workdir", omnet_config.omnet_workdir),
            ("omnet_ini", omnet_config.omnet_ini),
            ("omnetpp_root", omnet_config.omnetpp_root),
        ]:
            print(f"    {label}: {Path(path).exists()}  {path}")
        for idx, path in enumerate(omnet_config.omnet_ned_paths.split(";")):
            item = path.strip()
            if item:
                print(f"    omnet_ned_paths[{idx}]: {Path(item).exists()}  {item}")

    for benchmark in benchmarks:
        csv_path = resolve_csv_path(benchmark)
        graph = TaskGraph.from_csv(csv_path)
        original_assignment = extract_original_assignment(graph)
        original_pes = sorted(set(original_assignment.values()))
        make_original_static_tasks_mappable(graph)
        print(
            f"  {benchmark}: csv={csv_path} tasks={graph.num_tasks} "
            f"gb={len(graph.gb_task_ids)} mappable={len(graph.mappable_task_ids)} "
            f"original_pe_set={original_pes}"
        )


def write_summaries(output_dir: Path, records: list[dict[str, Any]]) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    write_csv_rows(output_dir / "runs_summary.csv", records)
    write_json(output_dir / "runs_summary.json", records)
    write_json(
        output_dir / "aggregate_summary.json",
        {
            "method": records[0].get("method", "thermal_rc_ls") if records else "thermal_rc_ls",
            "method_label": records[0].get("method_label", "ThermalRC-LS / Thermal-Resistance-Aware Local Search") if records else "ThermalRC-LS / Thermal-Resistance-Aware Local Search",
            "records": records,
        },
    )


def proxy_summary_text(benchmark: str, proxy: dict[str, Any]) -> str:
    original = proxy["original"]
    final = proxy[proxy["method"]]
    label = proxy["method_label"]
    return "\n".join([
        f"[{benchmark}] {label} proxy-only",
        f"  proxy score: {original['score']:.4f} -> {final['score']:.4f}",
        f"  Tmax proxy:  {original['Tmax_proxy']:.3f}K -> {final['Tmax_proxy']:.3f}K",
        f"  sigma proxy: {original['SigmaT_proxy']:.3f}K -> {final['SigmaT_proxy']:.3f}K",
        f"  hot proxy:   {original['HotCount_proxy']} -> {final['HotCount_proxy']}",
        comm_summary(original, final),
    ])


def full_summary_text(
    benchmark: str,
    original_metrics: dict[str, Any],
    rc_metrics: dict[str, Any],
    proxy: dict[str, Any],
    calibration_source: str,
) -> str:
    original_cost = cost_from_metrics(original_metrics)
    rc_cost = cost_from_metrics(rc_metrics)
    label = proxy["method_label"]
    return "\n".join([
        f"[{benchmark}] {label}",
        f"  calibration: {calibration_source}",
        f"  TR2 cost:    {original_cost:.4f} -> {rc_cost:.4f}",
        f"  proxy score: {proxy['original']['score']:.4f} -> {proxy[proxy['method']]['score']:.4f}",
        f"  T_max K:     {original_metrics['thermal']['T1_pe_peak_temp_K']:.3f} -> "
        f"{rc_metrics['thermal']['T1_pe_peak_temp_K']:.3f}",
        f"  sigma_T K:   {original_metrics['thermal']['T3_temp_std_K']:.3f} -> "
        f"{rc_metrics['thermal']['T3_temp_std_K']:.3f}",
        f"  hot PE:      {original_metrics['thermal']['T5_over_throttle_count']} -> "
        f"{rc_metrics['thermal']['T5_over_throttle_count']}",
        f"  makespan s:  {original_metrics['performance']['P1_makespan_s']:.6g} -> "
        f"{rc_metrics['performance']['P1_makespan_s']:.6g}",
    ])


def build_objective_weights(args: argparse.Namespace) -> RCObjectiveWeights:
    if args.objective == "thermal_only":
        return RCObjectiveWeights(
            objective="thermal_only",
            w_tmax=0.60,
            w_sigma=0.30,
            w_hot=0.10,
            w_comm=0.0,
        )
    return RCObjectiveWeights(
        objective="rc",
        w_tmax=args.rc_w_tmax,
        w_sigma=args.rc_w_sigma,
        w_hot=args.rc_w_hot,
        w_comm=args.rc_w_comm,
    )


def default_random_swap_rate(args: argparse.Namespace) -> float:
    if args.random_swap_rate is not None:
        return args.random_swap_rate
    if args.objective == "thermal_only":
        return 0.0
    return 0.10


def method_dir_name(weights: RCObjectiveWeights) -> str:
    return "thermal_only_rc_ls" if weights.objective == "thermal_only" else "thermal_rc_ls"


def method_label(weights: RCObjectiveWeights) -> str:
    return "ThermalOnly-RC-LS" if weights.objective == "thermal_only" else "ThermalRC-LS / Thermal-Resistance-Aware Local Search"


def comm_summary(original: dict[str, Any], final: dict[str, Any]) -> str:
    if "CommProxy" not in original or "CommProxy" not in final:
        return "  comm proxy:  not used by search objective"
    return f"  comm proxy:  {original['CommProxy']:.1f} -> {final['CommProxy']:.1f}"


if __name__ == "__main__":
    sys.exit(main())
